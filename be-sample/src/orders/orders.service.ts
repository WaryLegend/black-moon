  ...
  async create(userId: number, dto: CreateOrderDto): Promise<OrderResponseDto> {
    ...
    for (const item of dto.items) {
      ...
      if (!variant.product.price) {
        throw new BadRequestException('Missing product price');
      }

      const basePrice = variant.product.price.toNumber();
      const diffPrice = variant.differencePrice
        ? variant.differencePrice.toNumber()
        : 0;
      const unitPrice = basePrice + diffPrice;
