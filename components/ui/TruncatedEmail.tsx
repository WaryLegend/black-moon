...
export default function TruncatedEmail({ email = "", maxLength = 20 }) {
  ...
  const getDisplayEmail = () => {
    ...
    // If short enough, don't truncate
    if (email.length <= maxLength) return email;
    const allowedLocalLength = Math.max(
      3,
      maxLength - domain.length - 4, // space for "...@"
    );
    if (local.length <= allowedLocalLength) {
      return email;
    }
    return `${local.slice(0, allowedLocalLength)}...@${domain}`;
  };
