...
import { groupLabels, PAGE_SIZE } from "@/utils/constants";
import { priceFiltering } from "@/utils/price.filter";
...
export async function getProducts({ filters, page, sortBy }) {
  try {
    ...
    if (filters.basePrice) {
      filteredData = priceFiltering(filteredData, filters.basePrice);
    }
    // 4. SORT
    if (sortBy.field) {
      filteredData = sortData(filteredData, sortBy.field, sortBy.direction);
...
export async function getVariants({ filters, page, sortBy }) {
  try {
    ...
    if (filters.variantPrice) {
      filteredData = priceFiltering(filteredData, filters.variantPrice);
    }
    // 4. SORT
    if (sortBy.field) {
      filteredData = sortData(filteredData, sortBy.field, sortBy.direction);
